...
\begin{lem}\label{lem:interchange}
    ...
    \[i:(f' \o f)\smsh (g' \o g) \sim f' \smsh g' \o f \smsh g\]
    for pointed maps $f : A \to A'$, $f': A' \to A''$, $g : B \to B'$, $g' : B' \to B''$.
\end{lem}
\begin{proof}
	We first apply induction on the paths that all the maps in the statement respect the basepoint. We verify the underlying homotopy of $i$ by induction on terms $x$ of the domain $A \smsh B$ of the two maps; this can be defined on point constructors $(a,b)$, $\auxl$ and $\auxr$ to be the identity path. If $x$ varies over $\gluel_a$, we need to fill the following square:
	\begin{equation}\label{eq:i-gluel}
	\begin{tikzcd}
		(f'(f(a)), b''_0)
			\arrow[r,equals,"1"]
			\arrow[d,swap,equals,"\mapfunc{(f' \o f)\smsh (g' \o g)}(\gluel_a)"]
		& (f'(f(a)), b''_0)
			\arrow[d,equals,"\mapfunc{f' \smsh g' \o f \smsh g}(\gluel_a)"]
		\\
		\auxl
	...
	\end{tikzcd}
	\end{equation}
	This reduces to proving that $\mapfunc{(f'(f(a)),-)}(g'\o g)_0 \tr \gluel_{f'(f(a))} = \mapfunc{(f'(f(a)),-)}(\mapfunc{g'}g_0 \tr g'_0) \tr \gluel_{f'(f(a))}$. Since we assumed that $g_0$ and $g'_0$ are the identity path, the claim is easily verified. The case for $x$ varying over $\gluer_b$ is entirely analogous, giving the square:
	\begin{equation}\label{eq:i-gluer}
	TODO
	\end{equation}
	The resulting homotopy is pointed, as $i(a_0,b_0) \judgeq 1$ and the proofs that the two maps respect the basepoint are assumed to be the identity path.
\end{proof}
...
\end{lem}
\begin{proof}
	We will define the homotopy $t_g : 0 \smsh g$, with $0 : A \to A'$ and $g : B \to B'$; the definition for $t'_f$ is analogous. First, we apply induction on the path that $g$ respects the basepoint. The underlying homotopy of $t_g$ is given by induction on terms $x : A \smsh B$. On point constructors, we define:
	\begin{align*}
	t_g (a,b) &\defeq \gluer_{g(b)} \tr \gluer_{b'_0}\sy && : (a'_0, g(b)) = (a'_0, b'_0)\\
	t_g (\auxl) &\defeq \gluel_{a'_0}\sy && : \auxl = (a'_0, b'_0)\\
	t_g (\auxr) &\defeq \gluer_{b'_0}\sy && : \auxr = (a'_0, b'_0)
	\end{align*}
	If $x$ varies over $\gluel_a$, after some reductions, we need to fill the following square:
	\begin{equation}\label{eq:t-gluel}
	\begin{tikzcd}[column sep=7em]
		(a'_0, g(b_0))
			\arrow[r,equals,"\gluer_{b'_0} \tr \gluer_{b'_0}\sy"]
			\arrow[d,swap,equals, "\gluel_{a'_0}"]
		& (a'_0, b'_0)
			\arrow[d,equals,"1"]
		\\
		\auxl
			\arrow[r,swap,equals, "\gluel_{a'_0}\sy"]
		& (a'_0, b'_0)
	\end{tikzcd}
	\end{equation}
	...
	\begin{tikzcd}[column sep=7em]
		(a'_0, g(b_0))
			\arrow[r,equals,"\gluer_{g(b)} \tr \gluer_{b'_0}\sy"]
			\arrow[d,swap,equals, "\gluer_{g(b)}"]
		& (a'_0, b'_0)
			\arrow[d,equals,"1"]
		\\
		\auxr
			\arrow[r,swap,equals, "\gluer_{b'_0}\sy"]
		& (a'_0, b'_0)
	\end{tikzcd}
	\end{equation}
	The squares in (\ref{eq:t-gluel}) and (\ref{eq:t-gluer}) can both be filled by simple path algebra. The resulting homotopy is pointed, as $t_g(a_0,b_0)$ reduces to the identity path and the proof that $g$ respects the basepoint is also assumed to be the identity path.
\end{proof}
...
\begin{proof}
  ...
  $A_i$ and $B_i$ by $a_i$ and $b_i$ and we suppress the arguments of $\gluer$):
  \begin{equation}\label{eq:pent-left-ab}
    \begin{tikzcd}[column sep=5em]
      ...
      (a_3,g_2(g_1(b))) \arrow[r,swap,equals,"\gluer\tr\gluer\sy"] &
      (a_3,b_3)
    \end{tikzcd}
    \end{equation}  
   Now $\mapfunc{h\smsh k}(\gluer_z)=\gluer_{k(z)}$, so by general groupoid laws we see that the path on the bottom is equal to the path on the right, which means we can fill the square. For the other point constructors, the squares to fill are similar: if $x \judgeq \auxl$, we have:
    ...
    \begin{tikzcd}[column sep=5em]
      ...
      \arrow[d,swap,equals,"1"] &
      \auxr \arrow[d,equals,"\mapfunc{f_2\smsh g_2}(\gluel_{a_2}\sy)"] \\
      \auxr \arrow[r,swap, equals,"\gluel_{f_2(a_2)}\sy"] &
      (a_3,b_3)
    \end{tikzcd}
 \end{equation}
  ...
  \begin{equation}\label{eq:pent-left-auxr}
    \begin{tikzcd}[column sep=5em]
      ...
      \auxr \arrow[r,swap, equals,"\gluer_{g_2(b_2)}\sy"] &
      (a_3,b_3)
    \end{tikzcd}
  \end{equation}
	If $x$ varies over $\gluel_a$, we need to fill the cube:


  If $x$ varies over $\gluer_b$, we need to fill the cube below. The front and the back are
	the squares in (\ref{eq:pent-left-ab}) and (\ref{eq:pent-left-auxr}) respectively; the left square is degenerate; the other three sides are the squares in the definition of $i$ and $t$ to show that they respect $\gluer_b$ (given in (\ref{eq:i-gluer}) and (\ref{eq:t-gluer}) respectively), where we also apply $f_2\smsh g_2$ to the square on the right.
	\begin{center}
	\begin{tikzcd}
	& \auxr
		\arrow[rr, equals,"1"]
		\arrow[dd,equals,near start,"1"]
	&& \auxr
		\arrow[dd,equals,"\mapfunc{f_2\smsh g_2}(\gluer_{b_2}\sy)"]
	\\
	(f_2(a_2),g_2(g_1(b)))
		\arrow[rr, equals, near start, crossing over, "1"]
		\arrow[dd,equals,"1"]
		\arrow[ur,equals]
	&& (f_2(a_2),g_2(g_1(b)))
		\arrow[ur,equals]
	\\
	& \auxr
		\arrow[rr,equals,near start, "\gluer_{g_2(b_2)}\sy"]
	&& (a_3,b_3)
	\\
	(a_3,g_2(g_1(b)))
		\arrow[rr,equals,"\gluer\tr\gluer\sy"] \arrow[ur,equals] & &
	(a_3,b_3)
